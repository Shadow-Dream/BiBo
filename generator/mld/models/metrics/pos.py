...
class PosMetrics(Metric):
    def __init__(self, dist_sync_on_step=True):
        ...
        self.add_state("mpjpe_sum", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.add_state(
            "mpjpe_unaligned_sum", default=torch.tensor(0.0), dist_reduce_fx="sum"
        )
        self.add_state(
            "feature_error_sum", default=torch.tensor(0.0), dist_reduce_fx="sum"
        )

    def compute(self):
        metric = dict(
            MPJPE=self.mpjpe_sum / self.count,
            MPJPE_unaligned=self.mpjpe_unaligned_sum / self.count,
            FeaError=self.feature_error_sum / self.count,
        )
        return metric

    def update(self, joints_ref, joints_rst, feats_ref, feats_rst, lengths):
