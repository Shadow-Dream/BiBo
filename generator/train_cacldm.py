...
def main():
    ...
    test_dataloader.dataset.fixed_len = 60

    model = CACLDM(cfg, dataset)
    ...
    def validation(target_model, ema=False):
        ...
        for test_batch in tqdm(test_dataloader):
            ...
            loss_dict = model.allsplit_step("test", test_batch)
            for k, v in loss_dict.items():
                loss_value[k] = (
                    loss_value.get(k, 0)
                    + v.mean().item() * test_batch["motion"].shape[0]
                )
                loss_count[k] = loss_count.get(k, 0) + test_batch["motion"].shape[0]
        for k, v in loss_value.items():
            metrics[f"Metrics/{k}"] = v / loss_count[k] if loss_count[k] > 0 else 0.0
    ...
    while True:
        for step, batch in enumerate(train_dataloader):
            ...
            # Update training step for warmup/curriculum logic
