...
class BaseModel(nn.Module):
    ...
    def test_step(self, batch):
        ...
        if len(self.text_encoder_times) > 0:
            inference_aits_text = round(
                np.sum(self.text_encoder_times) / total_samples, 5
            )
            message += (
                f"Average Inference Time per Sentence [Text]: {inference_aits_text}\n"
            )

        if len(self.traj_encoder_times) > 0:
            inference_aits_hint = round(
                np.sum(self.traj_encoder_times) / total_samples, 5
            )
            message += (
                f"Average Inference Time per Sentence [Hint]: {inference_aits_hint}\n"
            )
        ...
        if len(self.vae_encode_times) > 0:
            inference_aits_vae_e = round(
                np.sum(self.vae_encode_times) / total_samples, 5
            )
            message += f"Average Inference Time per Sentence [VAE Encode]: {inference_aits_vae_e}\n"

        if len(self.vae_decode_times) > 0:
            inference_aits_vae_d = round(
                np.sum(self.vae_decode_times) / total_samples, 5
            )
    ...
    def configure_metrics(self):
        for metric in self.metric_list:
            if metric == "TM2TMetrics":
                ...
                self.ControlMetrics = ControlMetrics(
                    self.datamodule.name,
                    dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP,
                )
            elif metric == "PosMetrics":
                self.PosMetrics = PosMetrics(
                    dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP
                )
            elif metric == "PhysicalMetrics":
                # dataset_name: use the datamodule.name (e.g., 'humanml3d' or 'kit')
