...
def mld_collate(batch):
    ...
    mask = lengths_to_mask(
        adapted_batch["length"],
        adapted_batch["motion"].device,
        adapted_batch["motion"].shape[1],
    )
    adapted_batch["mask"] = mask

    # collate trajectory
    if notnone_batches[0][-1][0] is not None:
        adapted_batch["hint"] = collate_tensors(
            [torch.tensor(b[-1][0]).float() for b in notnone_batches]
        )
        adapted_batch["hint_mask"] = collate_tensors(
            [torch.tensor(b[-1][1]).float() for b in notnone_batches]
        )

    return adapted_batch


def mld_collate_motion_only(batch):
    batch = {
