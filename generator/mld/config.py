...
def get_module_config(cfg_model, paths, cfg_root):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="The input texts and lengths with txt format",
    )
    parser.add_argument(
        "--example_hint",
        type=str,
        required=False,
        help="The input hint ids and lengths with txt format",
    )
    parser.add_argument(
        "--no-plot",
        action="store_true",
        required=False,
        help="Whether to plot the skeleton-based motion",
    )
    parser.add_argument(
        "--replication",
        type=int,
        default=1,
        help="The number of replications of sampling",
    )
    parser.add_argument(
        "--optimize", action="store_true", help="Enable optimization for motion control"
    )
    parser.add_argument(
        "--wandb_name", type=str, default="", help="The name of the wandb run"
    )
